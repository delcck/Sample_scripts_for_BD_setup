...
foreach prefix $prefixesP {
    ...
    [atomselect top "noh and not (type FE)"] writepsf $molN.temp.psf
    [atomselect top "noh and not (type FE)"] writepdb $molN.temp.pdb
    mol delete top

    #set ID [mol new $molN.psf]
    #mol addfile $molN.pdb
    set ID [mol new $molN.temp.psf]
    mol addfile $molN.temp.pdb
    lappend IDs $ID
